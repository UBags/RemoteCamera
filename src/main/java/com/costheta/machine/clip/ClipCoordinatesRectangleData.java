...
public class ClipCoordinatesRectangleData {

    private static final Logger logger = LogManager.getLogger(ClipCoordinatesRectangleData.class);
    ...
    public static final void setMinDimensionsNeededForImage(int minDimensionsNeededForImage) {
        ClipCoordinatesRectangleData.minDimensionsNeededForImage = minDimensionsNeededForImage;
    }
    ...
    private LinkedHashMap<BaseImageProcessor, LinkedHashMap<String, Rectangle>> clippingRegionsByProcessor;

    public ClipCoordinatesRectangleData(InspectionPoint inspectionPoint) {
        ...
        this.clippingRegionsByProcessor = ClippingUtils.getClipBoxesForInspectionPoint(inspectionPoint);
...
//        }
        System.out.println("Reached here. inputImage = " + inputImage);
    }
    ...
    public int getAssessmentRegionsInProcessor(BaseImageProcessor processor) {
    ...
    public ArrayList<ClippingRectangle> getClippingRectanglesInProcessor(BaseImageProcessor processor) {
        ArrayList<ClippingRectangle> clippingRectangles = new ArrayList<>();
        LinkedHashMap<String, Rectangle> clips = clippingRegionsByProcessor.get(processor);
        if (clips == null) {
            logger.debug("Invalid processor. Processor " + processor + " with name " + processor.getName() + " not found in LinkedHashMap");
        }
        Iterator <Map.Entry<String, Rectangle>> entries = clips.entrySet().iterator();
        while (entries.hasNext()) {
            Map.Entry<String, Rectangle> entry = entries.next();
            ...
            try {
                ...
                logger.trace("Invalid assessment region number in getClippingRectanglesInProcessor()");
            }
            Rectangle rect = entry.getValue();
            ClippingRectangle thisClippingRectangle = new ClippingRectangle(id, key, rect);
            clippingRectangles.add(thisClippingRectangle);
        }
        return clippingRectangles;
    }

    public void setClippingRectangle(int processorIndex, int assessmentId, Rectangle rect) {
        Iterator<Map.Entry<BaseImageProcessor, LinkedHashMap<String, Rectangle>>> iterator = clippingRegionsByProcessor.entrySet().iterator();
        Map.Entry<BaseImageProcessor, LinkedHashMap<String, Rectangle>> entry = null;
        ...
        LinkedHashMap<String, Rectangle> clipBoxes = entry.getValue();
        Iterator<Map.Entry<String, Rectangle>> internalIterator = clipBoxes.entrySet().iterator();
        Map.Entry<String, Rectangle> aClipBox = null;

        for (int i = 1; (i <= assessmentId) && internalIterator.hasNext(); ++i) {
            aClipBox = internalIterator.next();
        }
        String key = aClipBox.getKey();
        aClipBox.setValue(rect);
    }

    public void setClippingRectangle(ClippingRectangle clippingRectangle) {
        Iterator<Map.Entry<BaseImageProcessor, LinkedHashMap<String, Rectangle>>> iterator = clippingRegionsByProcessor.entrySet().iterator();
        Map.Entry<BaseImageProcessor, LinkedHashMap<String, Rectangle>> entry = null;
        externalLoop: while (iterator.hasNext()) {
            ...
            while (internalIterator.hasNext()) {
                aClipBox = internalIterator.next();
                String key = aClipBox.getKey();
                if (key.equals(clippingRectangle.key)) {
                    aClipBox.setValue(clippingRectangle.clippingRectangle);
                    break externalLoop;
                }
            }
