...
public class SingleWordTextProcessor_LowLight extends BaseClipImageProcessor {

    private static final Logger logger = LogManager.getLogger(SingleWordTextProcessor_LowLight.class);
    private int runCounter = 1;
    private static int connectivity = 4;

    public SingleWordTextProcessor_LowLight(String name, ArrayList<String> patterns) {
        this(name, patterns, patterns.size());
        logger.trace("Leaving constructor");
    }

    public SingleWordTextProcessor_LowLight(String name, ArrayList<String> patterns, int assessmentRegions) {
    ...
    private ProcessingResult processForEach(Pix originalLargePix, Box clipBox, String stringPattern, Pattern pattern, int threadNumber) {
        ...
        BufferedImage originalImage = ImageUtils.convertPixToImage(originalReduced8);
        BufferedImage originalBinarized = OtsuThreshold.getLocalOtsuImageStandard(originalImage, originalImage.getWidth() / 2,originalImage.getHeight() / 3);
        imageWrite(runCounter + " - " + threadNumber + " - " + imageSerialCounter++ + " - originalBinarized.png", originalBinarized, BaseCamera.PNG, TraceLevel.TRACE);

        BufferedImage nbImage = ImageUtils.convertPixToImage(normBackPix);
        BufferedImage nbImageBinarized = OtsuThreshold.getLocalOtsuImageStandard(nbImage, originalImage.getWidth() / 2,originalImage.getHeight() / 3);
        imageWrite(runCounter + " - " + threadNumber + " - " + imageSerialCounter++ + " - nbImageBinarized.png", nbImageBinarized, BaseCamera.PNG, TraceLevel.TRACE);

        LeptUtils.dispose(originalReduced8);
        LeptUtils.dispose(normBackPix);

        BufferedImage binarizedImage = ImageUtils.imageGrayAnd(nbImageBinarized, originalBinarized);
