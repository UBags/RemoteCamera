...
public class RightSideTextProcessor extends BaseClipImageProcessor {

    private static final Logger logger = LogManager.getLogger(RightSideTextProcessor.class);
    private int imageCounter = 1;
    private static int connectivity = 4;

    public RightSideTextProcessor(String name, ArrayList<String> patterns) {
        super(name, patterns);
        logger.trace("After calling super() in constructor");
    }

    public RightSideTextProcessor(String name, ArrayList<String> patterns, int assessmentRegions) {
    ...
    public ProcessingResult process(BufferedImage image) {
        ...
        BufferedImage contNormImage = ImageUtils.convertPixToImage(contNormPix);
        // contNormImage = ImageUtils.relativeGammaAverageEnhancementSkipWhites(contNormImage, (int) (dimensions.getWidth() * 1.5), (int) (dimensions.getHeight() * 1.5), 216, ImageUtils.GAMMA_ENHANCEMENT_BELOW);
        contNormImage = ImageUtils.selectiveBlackening(contNormImage, (int) (dimensions.getWidth() * 0.67), (int) (dimensions.getHeight() * 0.67), 216, 1.2);
        imageWrite(imageCounter + " - " + imageSerialCounter++ + " - contNormGE.png", contNormImage, BaseCamera.PNG, TraceLevel.TRACE);

        Pix contNormGEPix = ImageUtils.getDepth8Pix(contNormImage);

        // Again, background normalise the contrast normalised image
        Pix normBackPix1 = Leptonica1.pixBackgroundNormFlex(contNormGEPix, 7, 7, 2, 2, 60); // 100
        pixWrite(imageCounter + " - " + imageSerialCounter++ + " - bn_cn_Pix.png", normBackPix1, ILeptonica.IFF_PNG, TraceLevel.TRACE);

        Pix BLG1 = Leptonica1.pixBilateralGray(normBackPix1, 10f, 25f, 16, 4);
        ...
        LeptUtils.dispose(contNormGEPix);
        ...
        Pix removeVertLines = ImageUtils.removeLines(removeHorLines, ImageUtils.VERTICAL_DIRECTION, (int) (dimensions.getHeight() + 8));
        ...
        ArrayList<ArrayList<Rectangle>> lines = getLines(rotatedPix, 0.375, 1.25, dimensions, imageCounter, imageSerialCounter++, TraceLevel.TRACE);
        ...
        lines = getLines(E2ECleaned, 0.375, 1.25, dimensions, imageCounter, imageSerialCounter++, TraceLevel.TRACE);
        ...
        double[] scalingFactors = new double[]{0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3};
