...
public class LeftTextProcessorForThinFont extends BaseClipImageProcessor {

    private static final Logger logger = LogManager.getLogger(LeftTextProcessorForThinFont.class);
    private int imageCounter = 1;
    private static int connectivity = 4;

    public LeftTextProcessorForThinFont(String name, ArrayList<String> patterns) {
        super(name, patterns);
    ...
    public ProcessingResult process(BufferedImage image) {
        ...
        BufferedImage baseImageMask = ImageUtils.relativeScaledGammaContrastEnhancementSkipWhites(baseImageInterimMask, (int) (dimensions.getWidth() * 2.0), (int) (dimensions.getHeight() * 2.0), 15, 242); // 10
...
*/
        ...
        // small kernel GE, based on average
        BufferedImage baseImageInterim = ImageUtils.relativeGammaAverageEnhancementSkipWhites(bnCnMasked, (int) (dimensions.getWidth() * 0.5), (int) (dimensions.getHeight() * 0.5),242, ImageUtils.GAMMA_ENHANCEMENT_EXTREME);
        // done twice, which is a difference from the sequence followed to create the mask
        baseImageInterim = ImageUtils.relativeGammaAverageEnhancementSkipWhites(baseImageInterim, (int) (dimensions.getWidth() * 0.5), (int) (dimensions.getHeight() * 0.5),242, ImageUtils.GAMMA_ENHANCEMENT_EXTREME);
        ...
        ArrayList<ArrayList<Rectangle>> lines = getLines(rotatedPix, 0.375, 2.5, dimensions, imageCounter, imageSerialCounter++, TraceLevel.TRACE);
        ...
        lines = getLines(E2ECleaned, 0.375, 2.5, dimensions, imageCounter, imageSerialCounter++, TraceLevel.TRACE);
