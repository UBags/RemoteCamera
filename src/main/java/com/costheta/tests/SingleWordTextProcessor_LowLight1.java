...
public class SingleWordTextProcessor_LowLight1 extends BaseClipImageProcessor {

    private static final Logger logger = LogManager.getLogger(SingleWordTextProcessor_LowLight1.class);
    private int runCounter = 1;
    private static int connectivity = 4;

    public SingleWordTextProcessor_LowLight1(String name, ArrayList<String> patterns) {
        this(name, patterns, patterns.size());
        logger.trace("Leaving constructor");
    }

    public SingleWordTextProcessor_LowLight1(String name, ArrayList<String> patterns, int assessmentRegions) {
    ...
    private ProcessingResult processForEach(Pix originalLargePix, Box clipBox, String stringPattern, Pattern pattern, int threadNumber) {
        ...
        // imageWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - geNorm3.png", nbImage, BaseCamera.PNG, TraceLevel.TRACE);

        // nbImage = ImageUtils.relativeGammaContrastEnhancement(nbImage,(int) dimensions.getWidth() / 2, (int) dimensions.getHeight() / 2);
//        nbImage = ImageUtils.relativeGammaContrastEnhancement(nbImage,5, 5);
//        imageWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - geNorm5.png", nbImage, BaseCamera.PNG, TraceLevel.TRACE);

//        Pix geNormBackPix = ImageUtils.getDepth8Pix(nbImage);
//        pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - geNormBackPix.png", geNormBackPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);
        // Pix bilateralGrayPix = Leptonica1.pixBilateralGray(normBackPix, (Math.min(normBackPix.w, normBackPix.h) - 5) * 1.0f, 50f, 16, 4);
        // pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - bilateralGrayPix.png", bilateralGrayPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);

//        Pix contNormPix = Leptonica1.pixContrastNorm(null, geNormBackPix, normBackPix.w / 2,
//                normBackPix.h / 2, 60, 2, 2); // 2, 2
//        pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - cnPix.png", contNormPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);

        // Pix sharpenedPix = Leptonica1.pixUnsharpMaskingGray(contNormPix, 5, 0.7f);
        // pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - sharpenedPix.png", sharpenedPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);
...
//        pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - Sauvola.png", binarized, ILeptonica.IFF_PNG, TraceLevel.TRACE);
...
//        LeptUtils.dispose(contNormPix);
        ...
        // pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - rotatedPix.png", rotatedPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);
        ...
        // pixWrite(threadNumber + " - " + imageCounter + " - " + imageSerialCounter++ + " - rotatedTesseractPix.png", tesseractRotatedPix, ILeptonica.IFF_PNG, TraceLevel.TRACE);
