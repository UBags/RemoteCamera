...
public class LeftTextProcessorForThickFont extends BaseClipImageProcessor {

    private static final Logger logger = LogManager.getLogger(LeftTextProcessorForThickFont.class);
    private int imageCounter = 1;
    private static int connectivity = 4;

    public LeftTextProcessorForThickFont(String name, ArrayList<String> patterns) {
        super(name, patterns);
    ...
    public ProcessingResult process(BufferedImage image) {
        ...
        BufferedImage baseImageMask = ImageUtils.relativeScaledGammaContrastEnhancementSkipWhites(baseImageInterimMask, (int) (dimensions.getWidth() * 2.0), (int) (dimensions.getHeight() * 2.0), 35, 242); // 10
        imageWrite(imageCounter + " - " + imageSerialCounter++ + " - baseImageForMask   .png", baseImageMask, BaseCamera.PNG, TraceLevel.TRACE);

        // ------ BASE IMAGE GOT For MASK --------
...
*/
        ...
        baseImageInterim = ImageUtils.relativeGammaAverageEnhancementSkipWhites(baseImageInterim, (int) (dimensions.getWidth() * 1.5), (int) (dimensions.getHeight() * 1.5),254, ImageUtils.GAMMA_ENHANCEMENT_EXTREME);
        imageWrite(imageCounter + " - " + imageSerialCounter++ + " - baseImageInterim.png", baseImageInterim, BaseCamera.PNG, TraceLevel.TRACE);

        // large kernel, 15 percentile, skip whites
        // changed from (2.0, 2.0, 15) which is for the mask
        // to (1.5, 1.5, 25)
        BufferedImage baseImage = ImageUtils.relativeScaledGammaContrastEnhancementSkipWhites(baseImageInterim, (int) (dimensions.getWidth() * 1.5), (int) (dimensions.getHeight() * 1.5), 35, 242);
